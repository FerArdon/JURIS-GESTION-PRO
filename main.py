...
def get_writable_base():
    ...
    # Si el proceso está compilado (PyInstaller frozen), SIEMPRE usar LocalAppData
    # El directorio _internal puede parecer escribible pero SQLite falla con WAL/journal
    ...
    # En modo desarrollo: usar BASE_DIR si es escribible
    if os.access(BASE_DIR, os.W_OK):
        return BASE_DIR

    # Fallback a LocalAppData si BASE_DIR no tiene permisos
    app_data = os.getenv('LOCALAPPDATA')
    if app_data:
        path = os.path.join(app_data, 'JURIS-GESTIÓN-PRO')
        os.makedirs(path, exist_ok=True)
...
LICENSE_PATH = os.path.join(WRITABLE_DIR, 'config', 'licencia.key')
...
# GESTIÓN DE LICENCIA
# ─────────────────────────────────────────────────────────────
def _leer_licencia_guardada():
    """Lee la clave de licencia persistida en disco."""
    try:
...
def _guardar_licencia(key):
    """Persiste la clave de licencia en disco."""
    try:
        ...
        logging.error(f'Error guardando licencia: {e}')
        return False

# ─────────────────────────────────────────────────────────────
# HTML DE ACTIVACIÓN (inline, sin archivo externo)
# ─────────────────────────────────────────────────────────────
ACTIVATION_HTML = '''
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <title>Activacion - JURIS-GESTION-PRO</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{
      font-family:"Segoe UI",sans-serif;
      background:linear-gradient(135deg,#0a0f1e 0%,#1a2540 100%);
      color:#e0e6f0;height:100vh;display:flex;
    ...
    h1{font-size:21px;font-weight:700;color:#4FC3F7;margin-bottom:4px}
    .sub{color:#78909C;font-size:13px;margin-bottom:26px}
    .hw-box{
    ...
    .hw-label{font-size:11px;color:#546E7A;text-transform:uppercase;letter-spacing:.6px}
    .hw-val{font-size:17px;font-weight:700;color:#4FC3F7;
            font-family:"Courier New",monospace;margin-top:3px;letter-spacing:2px}
    input{
      ...
      letter-spacing:3px;outline:none;margin-bottom:14px;
    ...
    button:hover{background:linear-gradient(135deg,#1976D2,#1565C0);transform:translateY(-1px)}
    button:disabled{opacity:.5;cursor:not-allowed;transform:none}
    .msg{margin-top:14px;font-size:13px;min-height:20px;transition:all .2s}
    .err{color:#EF5350}.ok{color:#66BB6A}
    .copy-btn{
      font-size:11px;background:rgba(79,195,247,.15);
      border:1px solid rgba(79,195,247,.3);color:#4FC3F7;
...
<div class="card">
  <div class="icon">&#9878;</div>
  <h1>JURIS-GESTION-PRO</h1>
  <p class="sub">Se requiere activacion de licencia para continuar</p>
  <div class="hw-box">
    <div class="hw-label">Hardware ID de este equipo
      <button class="copy-btn" onclick="copiarHW()">Copiar</button>
    </div>
    <div class="hw-val" id="hw">Cargando...</div>
  </div>
  <input type="text" id="key" placeholder="XXXXX-XXXXX-XXXXX-XXXXX"
         maxlength="23" oninput="fmt(this)" onkeydown="if(event.key===\'Enter\')activar()">
  <button id="btn" onclick="activar()">Activar Software</button>
  <div class="msg" id="msg"></div>
</div>
<script>
async function init(){
  try{
    const h=await window.pywebview.api.get_hardware_id();
    document.getElementById(\'hw\').textContent=h;
  }catch(e){document.getElementById(\'hw\').textContent=\'Error\';}
}
function copiarHW(){
  const t=document.getElementById(\'hw\').textContent;
  navigator.clipboard.writeText(t);
}
function fmt(el){
  let v=el.value.replace(/[^A-Za-z0-9]/g,\'\').toUpperCase();
  let r=\'\';
  for(let i=0;i<v.length&&i<20;i++){
    if(i>0&&i%5===0)r+=\'-\';
    r+=v[i];
  }
  el.value=r;
...
async function activar(){
  ...
  if(key.replace(/-/g,\'\').length<20){
    msg.className=\'msg err\';msg.textContent=\'Ingrese los 20 caracteres de la clave.\';
    return;
  }
  btn.disabled=true;msg.className=\'msg\';msg.textContent=\'Verificando...\';
  const r=await window.pywebview.api.activar(key);
  if(r.ok){
    msg.className=\'msg ok\';
    msg.textContent=\'Licencia activada. Iniciando aplicacion...\';
  }else{
    msg.className=\'msg err\';msg.textContent=r.mensaje;
    btn.disabled=false;
  }
}
...
class LicenciaAPI:
    """API puente para la ventana de activacion de licencia."""
    def __init__(self):
        self.activacion_exitosa = False

    def get_hardware_id(self):
        from src.licencia import get_hardware_id
        return get_hardware_id()

    def activar(self, clave):
        from src.licencia import verificar_licencia
        es_valida, mensaje, dias = verificar_licencia(clave)
        if es_valida:
            if _guardar_licencia(clave):
                self.activacion_exitosa = True
                import threading
                threading.Timer(1.4, lambda: webview.windows[0].destroy()).start()
                return {'ok': True, 'mensaje': mensaje, 'dias': dias}
...
class JurisAPI:
    ...
    def get_licencia_info(self):
        """Retorna el estado de la licencia activa para mostrar en la UI."""
        from src.licencia import verificar_licencia
        key = _leer_licencia_guardada()
        es_valida, mensaje, dias = verificar_licencia(key)
        return {
            'es_valida':    es_valida,
            'mensaje':      mensaje,
            'dias_restantes': dias,
            'es_permanente':  dias >= 9999,
        }

    def activar_licencia(self, clave):
        """Activa una nueva licencia desde la pantalla de configuracion."""
        from src.licencia import verificar_licencia
        es_valida, mensaje, dias = verificar_licencia(clave)
        if es_valida:
            _guardar_licencia(clave)
            return {'ok': True, 'mensaje': mensaje, 'dias': dias}
        return {'ok': False, 'mensaje': mensaje}
...
if __name__ == '__main__':
    ...
    # ── 1. VERIFICAR LICENCIA ANTES DE LANZAR LA APP ──────────────────
    from src.licencia import verificar_licencia
    _key = _leer_licencia_guardada()
    _es_valida, _msg, _dias = verificar_licencia(_key)

    if not _es_valida:
        logging.info('Licencia no válida — mostrando ventana de activación.')
        _lic_api = LicenciaAPI()
        webview.create_window(
            title='Activación — JURIS-GESTIÓN-PRO',
        ...
        # Si el usuario cerró sin activar → salir
        if not _lic_api.activacion_exitosa:
            logging.info('Activación cancelada. Cerrando aplicación.')
            sys.exit(0)

        # Re-verificar después de activar
        _key = _leer_licencia_guardada()
        _es_valida, _msg, _dias = verificar_licencia(_key)
        if not _es_valida:
            sys.exit(0)

    logging.info(f'Licencia válida: {_msg} ({_dias} días restantes)')

    # ── 2. LANZAR APLICACIÓN PRINCIPAL ────────────────────────────────
    api = JurisAPI()
