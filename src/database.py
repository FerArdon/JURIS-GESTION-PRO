...
def _resolver_db_path():
    """
    Resuelve la ruta de la BD de forma segura tanto en desarrollo como en EXE instalado.
    - EXE compilado (frozen): usa %LOCALAPPDATA%\JURIS-GESTIÓN-PRO\data\
    - Desarrollo: usa la carpeta raíz del proyecto (padre de src/)
    """
    # Si main.py ya calculó DB_PATH y lo expuso como variable de módulo, usarlo directamente
    import __main__
    if hasattr(__main__, 'DB_PATH'):
        return __main__.DB_PATH

    # Fallback: mismo algoritmo que main.py
    if getattr(sys, 'frozen', False):
        app_data = os.getenv('LOCALAPPDATA', '')
        base = os.path.join(app_data, 'JURIS-GESTIÓN-PRO') if app_data else os.path.dirname(sys.executable)
    else:
        # En desarrollo: subir un nivel desde src/
        base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    data_dir = os.path.join(base, 'data')
    os.makedirs(data_dir, exist_ok=True)
    return os.path.join(data_dir, 'juris_gestion_pro.db')
...
def inicializar_db():
    ...
    # Asegurar que el directorio 'data' exista (nunca dentro de Program Files)
    os.makedirs(DATA_DIR, exist_ok=True)
